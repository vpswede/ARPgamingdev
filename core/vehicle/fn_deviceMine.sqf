...
_resourceZones = ["apple_1","apple_2","apple_3","apple_4","peaches_1","peaches_2","peaches_3","peaches_4","heroin_1","cocaine_1","weed_1","lead_1","lead_2","iron_1","salt_1","sand_1","diamond_1","diamond_2","oil_1","oil_2","rock_1","zinc_1","zinc_2","silver_1","silver_2","frog_1","ecstasy_1","mushroom_1","sugar_1","sugar_2","rice_1","rice_2"];
_zone = "";
...
_item = switch(true) do {
	...
	case (_zone in ["lead_1","lead_2"]): {"copper_unrefined"};
	case (_zone in ["iron_1"]): {"iron_unrefined"};
	case (_zone in ["salt_1"]): {"salt_unrefined"};
	case (_zone in ["sand_1"]): {"sand"};
	case (_zone in ["diamond_1","diamond_2"]): {"diamond_uncut"};
	case (_zone in ["oil_1","oil_2"]): {"oil_unprocessed"};
	case (_zone in ["zinc_1","zinc_2"]): {"zinc_unprocessed"};
	case (_zone in ["silver_1","silver_2"]): {"silver_unprocessed"};
	case (_zone in ["frog_1"]): {"frog"};
	case (_zone in ["ecstasy_1"]): {"ecstasy_unprocessed"};
	case (_zone in ["mushroom_1"]): {"mushroom_unprocessed"};
	case (_zone in ["sugar_1","sugar_2"]): {"sugar_unprocessed"};
	case (_zone in ["rice_1","rice_2"]): {"rice_unprocessed"};
	default {""};
};
