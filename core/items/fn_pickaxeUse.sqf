...
switch (true) do {
	...
	case (player distance (getMarkerPos "lead_2") < 30): {_mine = ["copper_unrefined",2];};
	case (player distance (getMarkerPos "iron_1") < 30): {_mine = ["iron_unrefined",2];};
	case (player distance (getMarkerPos "iron_2") < 30): {_mine = ["iron_unrefined",2];};
	...
	case (player distance (getMarkerPos "zinc_1") < 50): {_mine = ["zinc_unprocessed",2];};
	case (player distance (getMarkerPos "zinc_2") < 50): {_mine = ["zinc_unprocessed",2];};
	case (player distance (getMarkerPos "silver_1") < 50): {_mine = ["silver_unprocessed",2];};
	case (player distance (getMarkerPos "silver_2") < 50): {_mine = ["silver_unprocessed",2];};
	default {""};
};
//Mine check
