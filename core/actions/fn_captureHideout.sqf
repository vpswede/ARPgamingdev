...
_title = localize "STR_GNOTF_CapturingHideout";
...
[[[0,1],"STR_GNOTF_CaptureSuccess",true,[name player,(group player) GVAR "gang_name"]],"life_fnc_broadcast",true,false] call life_fnc_MP;
_hideout SVAR["inCapture",false,true];
_hideout SVAR["gangOwner",grpPlayer,true];
[[[0,1],format[localize "STR_GNOTF_CaptureSuccess",name player,(group player) GVAR "gang_name" ]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;

// CREATE MARKER AT MAP BY Pictureclass
 
_markername = str(getPos _hideout);
_gangname2 = formatText["Captured by: %1",(group player) GVAR "gang_name"];
