...
switch (true) do {
	...
	case (_item in ["apple","rabbit","salema","ornate","mackerel","tuna","mullet","catshark","turtlesoup","hen","rooster","sheep","goat","donuts","tbacon","peach","sandwich","twister"]): {
    ...
    case (EQUAL(_item,"LSD")): {
		[] spawn life_fnc_lsd;
	};

    case (EQUAL(_item,"ecstasy_processed")): {
		[] spawn life_fnc_ecstasy;
	};

	case (EQUAL(_item,"mushroom_processed")): {
		[] spawn life_fnc_mushroom;
	};
	...
	case (EQUAL(_item,"cider")): {
		[] spawn life_fnc_cider;
	};
