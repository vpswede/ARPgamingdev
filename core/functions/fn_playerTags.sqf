...
_headgear = ["H_Shemag_olive","H_Shemag_khk","H_ShemagOpen_tan","H_ShemagOpen_khk","G_Balaclava_blk","G_Balaclava_oli"];
	...
	if(!(lineIntersects [eyePos player, eyePos _x, player, _x]) && {!isNil {_x GVAR "realname"}}) then {
		...
		if(count _sPos > 1 && {_distance < 15}) then {
			_text = switch (true) do {
				case ((headgear _x) in _headgear && (group _x != group player)): {format["<t color='#000000'>Masked Player</t>"];};
				...
				case (!isNil {(_x GVAR "rank")}): {format["<img image='%1' size='1'></img> %2",switch ((_x GVAR "rank")) do {
					case 2: {"icons\Police\r2.paa"}; 
					case 3: {"icons\Police\r3.paa"};
					case 4: {"icons\Police\r4.paa"};
					case 5: {"icons\Police\r5.paa"};
					case 6: {"icons\Police\r6.paa"};
					case 7: {"icons\Police\r7.paa"};
					case 8: {"icons\Police\r2.paa"};
					default {"icons\Police\r1.paa"};
			...
			if(_x getVariable ["DirectVON",false]) then {_text =
			 "<t color='#00FF88'>[TALKING] " + _text};
